...
const CardTicketComponent = ({ title, date, time, price, image, drink, onPress, boat, remainingTimeSeconde }) => {
  const initialTimeInSeconds = 24 * 60 * 60;
  const [remainingTime, setRemainingTime] = useState(remainingTimeSeconde);
  ...
  return (
    <TouchableOpacity style={{ marginBottom: 40 }} onPress={onPress} >
      <View style={{ borderRadius: 5, overflow: 'hidden', marginHorizontal: 25 }}>
        ...
        <View style={{ position: 'absolute', width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: COLORS.white, height: 45 }}>
          ...
          <Animated.View style={[{ flexDirection: 'row', alignItems: 'center', marginRight: 10 }, animatedStyles]}>
            <Text style={[{ color: COLORS.tertary, fontFamily: 'MonumentExtended-Ultrabold', fontSize: 14, marginRight: 5, textTransform: 'uppercase' }, timerStyle]}>
      ...
      <Text style={{ marginHorizontal: 25, color: COLORS.white, fontFamily: 'MonumentExtended-Ultrabold', textTransform: 'uppercase', fontSize: SIZES.large, marginTop: 10 }}>{title}</Text>
      <View style={{ flexDirection: 'row', marginHorizontal: 25, marginTop: 5 }}>
        {boat === '' ? <Text style={{ color: COLORS.gray, fontFamily: 'SpaceGrotesk-Bold', fontSize: SIZES.medium, textTransform: 'uppercase' }}>{drink} Drink
        </Text> : <Text style={{ color: COLORS.gray, fontFamily: 'SpaceGrotesk-Bold', fontSize: SIZES.medium, textTransform: 'uppercase' }}>{drink}
        </Text>}
