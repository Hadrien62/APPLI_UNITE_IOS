...
const CardPassComponent = (tempsRestant) => {
    const [iconColor, setIconColor] = useState(COLORS.tertary);
    const [isTimerEnding, setIsTimerEnding] = useState(false);
    const [remainingTime, setRemainingTime] = useState(tempsRestant.tempsRestant);
    ...
    return (
        <TouchableOpacity onPress={BraceletScreen} style={{ marginBottom: 40 }}>
            <View style={{ borderRadius: 5, overflow: 'hidden', marginHorizontal: 25 }}>
                <Image source={require('../assets/images/MaltaPass.png')} style={{ width: videoWidth, height: videoHeight, resizeMode: 'cover' }} />
            ...
            <Text style={{ marginHorizontal: 25, color: COLORS.white, fontFamily: 'MonumentExtended-Ultrabold', textTransform: 'uppercase', fontSize: SIZES.large, marginTop: 10 }}>Freedom malta pass</Text>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginHorizontal: 25 }}>
                <Text style={{ color: COLORS.secondary, fontFamily: 'SpaceGrotesk-Bold', textTransform: 'uppercase', fontSize: SIZES.xMedium }}>available For Life</Text>
                <View style={{ height: 35, paddingHorizontal: 15, flexDirection: 'row', borderRadius: 5, justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.tertary, marginTop: -5, }}>
                    <Text style={{ color: COLORS.white, fontFamily: 'SpaceGrotesk-Bold', fontSize: SIZES.large }} >27€</Text>
                </View>
            </View>
        </TouchableOpacity>
    );
...
export default CardPassComponent;